#ifndef TAILOREDSKETCH_CODE_CU_SKETCH_H
#define TAILOREDSKETCH_CODE_CU_SKETCH_H
...
public:
    CU_Sketch(int _w, int _d, int _hash_seed = 1000){
        w = _w/4/_d, d = _d;
    ...
    void Insert(const char* str){
        ...
        if (min_value == COUNTER_SIZE_MAX_CNT)return;
        for (int i = 0; i < d; i++){
...
class CU_Sketch_HS:public Sketch {
private:
    char  hash_size;
...
public:
    CU_Sketch_HS(uint32_t _w, uint32_t _d, uint32_t _hash_seed = 1000) {
        d             	= _d;
    ...
    void Insert(const char* str) {
        ...
        int temp = 0, min_value = (1 << (COUNTER_SIZE - 1)) - 1;
        for(char i = 0;i < d;i++){
            int addr = index + (i << bias_range) + (hash_value & bias_mask);
            hash_value >>= bias_range;
            temp=Ask(addr,counter+(i*((w + ((d - 1) << bias_range) + 0x10))));
            min_value = temp < min_value ? temp : min_value;
        }
        hash_value = hash_func(str, KEY_LEN, hash_seed);
        index = ((hash_value & index_mask) << bias_range);
        tmp = index - w;
        index = tmp < 0?index : tmp;
        hash_value >>= index_range;
        for (int i = 0; i < d; i++){
            int addr = index + (i << bias_range) + (hash_value & bias_mask);
            hash_value >>= bias_range;
            if (Ask(addr,counter+(i*((w + ((d - 1) << bias_range) + 0x10)))) == min_value)
...
class CU_Sketch_PQ:public Sketch {
private:
    char  clk,hash_size;
    unsigned int*counter;
    ...
    uint64_t (*hash_func)(const void*, int32_t, uint32_t);
public:
    CU_Sketch_PQ(uint32_t _w, uint32_t _d, uint32_t _hash_seed = 1000) {
        clk		= 0;
        d             	= _d;
        hash_size	= (d == 2) ? 0x20 : 0x40;
        hash_func	= (d == 2) ? MurmurHash32 : MurmurHash64B;
    ...
    void Insert(const char* str) {
        ...
        for(char i = 0;i < d;i++){
            ...
            hash_value >>= bias_range;
            temp=Ask(prefch[clk][i],counter+(i*((w + ((d - 1) << bias_range) + 0x10))));
            min_value = temp < min_value ? temp : min_value;
        }
        for (int i = 0; i < d; i++){
            int addr = prefch[clk][i];
            if (Ask(addr,counter+(i*((w + ((d - 1) << bias_range) + 0x10)))) == min_value)
                Add(addr,counter+(i*((w + ((d - 1) << bias_range) + 0x10))));
        }
    }
...
#endif //TAILOREDSKETCH_CODE_CU_SKETCH_H
